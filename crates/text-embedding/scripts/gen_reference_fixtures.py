...
sess = ort.InferenceSession(f"{SP}/egemma/onnx/model_quantized.onnx", providers=["CPUExecutionProvider"])
...
print("TRAP_COSINE(q vs d same text):", float(trap_q @ trap_d / (np.linalg.norm(trap_q)*np.linalg.norm(trap_d))))

def trunc512(v):
    t = v[:512]; return t / np.linalg.norm(t)
...
fx = {
  ...
  "query_prefix": QP, "doc_prefix": DP, "dim_full": 768, "dim_truncated": 512,
  "sentences": sents,
...
with open(f"{SP}/reference_embeddings.json", "w") as f: json.dump(fx, f)
print("FIXTURE_WRITTEN", len(sents), "sents;", "bytes:", __import__("os").path.getsize(f"{SP}/reference_embeddings.json"))
