...
pub(crate) async fn get_secret<R: tauri::Runtime>(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        crate::secrets::get_secret_blocking(&app, &scope, &key)
    })
    .await
...
pub(crate) async fn set_secret<R: tauri::Runtime>(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        crate::secrets::set_secret_blocking(&app, &scope, &key, &value)
    })
    .await
    .map_err(|error| error.to_string())?
}
...
pub(crate) async fn delete_secret<R: tauri::Runtime>(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        crate::secrets::delete_secret_blocking(&app, &scope, &key)
    })
    .await
    .map_err(|error| error.to_string())?
}
